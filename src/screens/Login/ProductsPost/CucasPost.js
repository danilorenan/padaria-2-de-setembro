...
import CucasPosList from './CucasPostList';
    ...
    return (
        <ProductsPostContainer className={classes.root}>
            ...
            { !!file.length && file.map(item => {
                return <CucasPosList setLoad={setLoad} handleDelete={handleDelete} file={item} key={item.id}/>
            }) }
        </ProductsPostContainer>
    )
}

export default CucasPost;
